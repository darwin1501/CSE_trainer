...
const TrainingResultViewer = props => {
  ...
  const [answeredQuestionToView, setansweredQuestionsToView] = useState(
  ...
  return (
    <div className='flex flex-column align-center'>
      <div className={TrainingResultViewerStyle.main_score_container}>
        <p className={TrainingResultViewerStyle.text_main_score}>
          {getScoreInPercentage(mainScore, totalQuestionCount)}%
        </p>
        <p className={TrainingResultViewerStyle.subtext_main_score}>
          ({mainScore} / {totalQuestionCount})
        </p>
      </div>
      <h4 style={{ margin: '0' }}>You Score</h4>
      {numericalCount > 0 && (
        <p>
          Numerical: {numerical} / {numericalCount} (
          {getScoreInPercentage(numerical, numericalCount)}%)
        </p>
      )}
      {analyticalCount > 0 && (
        <p>
          <b>Analytical:</b> {analytical} / {analyticalCount} (
          {getScoreInPercentage(analytical, analyticalCount)}%)
        </p>
      )}
      {verbalCount > 0 && (
        <p>
          <b>Verbal:</b> {verbal} / {verbalCount} (
          {getScoreInPercentage(verbal, verbalCount)}%)
        </p>
      )}
      {philConsCount > 0 && (
        <p>
          <b>Philippine Constitution:</b> {philConstitution} / {philConsCount} (
          {getScoreInPercentage(philConstitution, philConsCount)}%)
        </p>
      )}
      {clericalCount > 0 && (
        <p>
          <b>Clerical:</b> {clerical} / {clericalCount} (
          {getScoreInPercentage(clerical, clericalCount)}%)
        </p>
      )}
      ...
      <div style={{ marginTop: '80px' }}>
        ...
        <div
          className={`flex flex-column gap-md ${TrainingResultViewerStyle.answered_question_container}`}
          style={{ marginTop: '20px', marginBottom: '15%' }}
        >
          {answeredQuestions}
        </div>
      </div>
